...
func (chunkStream *ChunkStream) writeChunk(w *ReadWriter, chunkSize int) error {
	...
	if (chunkStream.Data == nil || len(chunkStream.Data) == 0) && chunkStream.DataSlices != nil && len(chunkStream.DataSlices) > 0 {
		return chunkStream.writeFrameSlices(w, chunkSize)
	}
...
func (chunkStream *ChunkStream) writeFrameSlices(w *ReadWriter, chunkSize int) error {
	//chunkStream.StreamID = chunkStream.CSID
	chunkStream.Length = 0
	slices := chunkStream.DataSlices
	for _, slice := range slices {
		chunkStream.Length += uint32(len(slice.Data))
	}
	totalLen := uint32(0)
	numChunks := (chunkStream.Length / uint32(chunkSize))
	sliceIndex := 0
	...
	for i := uint32(0); i <= numChunks; i++ {
		...
		inc := uint32(chunkSize)
		start := uint32(i) * uint32(chunkSize)
		if chunkStream.Length-start <= inc {
			inc = chunkStream.Length - start
		}
		if inc==0{
			continue
		}
		...
		data := slices[sliceIndex].Data[sliceoffset:]
		if uint32(len(data)) < inc {
			leave := int(inc)
			buf := []byte{}
			for leave > 0 {
				data = slices[sliceIndex].Data[sliceoffset:]
